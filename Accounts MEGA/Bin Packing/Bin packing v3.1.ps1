...
If ( (Test-Path -Path ($FolderBrowser.SelectedPath+'\Packs') ) -eq $False){
    ...
    If ($?){Write-Warning "Cartella 'Packs' creata."} Else {Write-Warning "Errore nella creazione della cartella 'Packs'."; Pause}
} Else {
...
[System.Collections.ArrayList]$AllItems = @($Items) #PROVA
...
Do {$i++
    ...
    If ($?){Write-Warning ("Nuovo pack '" + $FolderName + "' creato.")}
