...
New-Item -Path $FolderBrowser.SelectedPath -ItemType "Directory" -Name $FolderName @DebugMode | Out-Null
...
Do {$Items = @($Items | Where-Object -Property "Size" -LE $AvailableSpace | Sort-Object -Property "Size" -Descending)
    ...
    If ($Items.Count -gt 0){ #Altrimenti finisco oltre i limiti della matrice
        Move-Item -Path $Items[0].FullName -Destination ($FolderBrowser.SelectedPath+'\'+$FolderName) @DebugMode | Out-Null
...
If ($AvailableSpace/1MB -ge 1024){
    Write-Warning ("Spazio rimanente: " + [Math]::Round($AvailableSpace/1GB, 2) + " GB su " + "$Storage $Unit (" + [Math]::Round($AvailableSpace/([string]$Storage+$Unit) * 100, 4) + "%)")
} Else {Write-Warning ("Spazio rimanente: " + [Math]::Round($AvailableSpace/1MB, 2) + " MB su " + "$Storage $Unit (" + [Math]::Round($AvailableSpace/([string]$Storage+$Unit) * 100, 4) + "%)")}